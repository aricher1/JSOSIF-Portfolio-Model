...
class Portfolio:
    ...
    def get_sharpe_ratio(self, include_cash:bool=True, freq='ME', start=ONE_YR, end=NOW, rf_override=None):
        ...
        Args:
            ...
            freq (str): Frequency of returns data ('D' = daily, 'W' = weekly, 'ME' = monthly).
    ...
    # Reviewed: 2025-06-27
    def get_betas(self, historical:bool=False, include_cash:bool=True, freq='ME', start=FIVE_YRS, end=NOW):
        """
...
def get_dual_betas(ticker, benchmark="^GSPC", freq='ME', start=FIVE_YRS, end=NOW):
    ...
    Returns:
        dict: {'PLUS': float, 'MINUS': float} containing upside and downside beta values.
    """
    if (ticker,'PLUS',benchmark,freq,start,end) in equity_betas: #checks if dual betas have already been calculated for ticker
        return {
...
def plot_beta_scatter(ticker, benchmark='^GSPC', freq='ME', start=FIVE_YRS, end=NOW, title="Portfolio", download_graph=False):
    ...
    Parameters:
        ...
        freq (str): Frequency of returns ('D', 'W', 'ME', etc.).
...
def print_risk_report(holding: Holding, portfolio: Portfolio):
    """
    Prints a formatted risk report for the holding with statistics and correlations.
    ...
    - Risk tier
    - Suggested buy values (min, med, max)
    - Correlation to portfolio, sector, and other holdings (significant only)

    Parameters:
        holding (Holding): Holding to report on.
        portfolio (Portfolio): Portfolio containing the holding.
    """
    ...
    min_buy_value = buy_amounts["MIN"] * price
    med_buy_value = buy_amounts["MED"] * price
    max_buy_value = buy_amounts["MAX"] * price
    ...
    print(f"┌Kurtosis\n└ {fmt(kurtosis)}")
    print(f"┌Risk Tier\n└ {risk_tier}")
    print(f"┌Min Buy Value\n└ ${min_buy_value:,.2f}")
    print(f"┌Med Buy Value\n└ ${med_buy_value:,.2f}")
    print(f"┌Max Buy Value\n└ ${max_buy_value:,.2f}")
    print(f"┌Portfolio Correlation")
    print(f"└ {fmt(port_corr, pct=True)}")
    print(f"┌Sector Correlation")
    print(f"└ {fmt(sector_corr, pct=True)}")
